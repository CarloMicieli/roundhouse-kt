...
class GlobalErrorHandlingTest(@Autowired private val webClient: WebTestClient) {
    ...
    fun `should produce a problem detail response when an unhandled exception is thrown`() {
        webClient.get()
            ...
            .expectBody()
            .jsonPath("$.detail").isEqualTo("An error has occurred")
            .jsonPath("$.instance").isNotEmpty
            .jsonPath("$.timestamp").isNotEmpty
            .jsonPath("$.title").isEqualTo("Internal Server Error")
            .jsonPath("$.type").isEqualTo("trn:problem-type:internal-server-error")
    }
}
...
class TestController {
    @GetMapping("/ops")
    suspend fun post(): String = throw RuntimeException("Ops, something bad happened")
}
